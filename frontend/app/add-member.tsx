...
export default function AddMember() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      show(e instanceof ApiError ? e.message : "Failed to add member", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.surface }}>
      ...
      >
        ...
        <Field label="Mobile Number" placeholder="10-digit mobile" icon="call-outline" keyboardType="phone-pad" value={mobile} onChangeText={setMobile} testID="member-mobile-input" />
        ...
        <View style={styles.rowSplit}>
          ...
          <View style={{ flex: 1 }}>
            <Field label="Advance (₹)" placeholder="0" keyboardType="numeric" value={advance} onChangeText={setAdvance} testID="member-advance-input" />
          </View>
        </View>
        ...
        <Button title="Add Member" onPress={submit} loading={loading} icon="checkmark" testID="member-submit-button" />
      </KeyboardAwareScrollView>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  note: { fontFamily: fonts.regular, fontSize: fontSize.sm, color: colors.muted, lineHeight: 18 },
});
